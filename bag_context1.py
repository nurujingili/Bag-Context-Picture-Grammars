...
"""
def update_vector(rule,vector):
    change=rule[4]
    #vector=np.add(change, vector)
    vector=tuple(p+q for p, q in zip(change, vector))
    print("Vector:",vector)
    return vector
...
@:param forbidding - forbidding context
"""
def apply_lower_and_upper_limit(rule,vector):
    lower_limit=rule[2]
    upper_limit=rule[3]
    change=rule[4]
    check=[False]
    results=[x<=z<=y for x,y,z in zip(lower_limit,upper_limit,vector)]

    print("results:",results)
    if not set(results)&set(check):
        return True
"""
...
"""
font = ImageFont.truetype("Arial Narrow.ttf", 40)
...
def drawPictorialForm(im, pformV):
    for sq in pformV:
        ...
        for label in sq.keys():
            ...
            if (label == "b"):
                draw.ellipse((x1, y2, x2, y1), fill="magenta",)
            if (label == "g"):
                draw.ellipse((x1, y2, x2, y1), fill= "green")
            if (label == "y"):
                    draw.ellipse((x1, y2, x2, y1), fill="green", )
            tx1 = x1+((x2-x1)/2)
            ty1 = y1 + ((y2-y1)/2)
            ...
            if label != "w":
                if label != "b":
                    if label!="g":
                        if label != "y":
                                        draw.text((tx1-5, ty1-5), label, font=font, fill="black")
...
if __name__ == "__main__":
    rules = control_bag
   # userInput("Generate the initial pictorial form? (Y/N)")
    ...
    del draw
    ## i to keep track of number of images produced
    i = 1
